...
function showDingTalkConfig() {
  ...
  if (area.style.display === 'none') {
    ...
    if (input) {
      ...
      fetch(HiEnglish.getServerUrl() + '/api/dingtalk-config?token=' + encodeURIComponent(HiEnglish.currentToken()), { cache: 'no-store' }).then(function(r) { return r.json(); }).then(function(data) {
...
function confirmResetPassword() {
  ...
  fetch(HiEnglish.getServerUrl() + '/api/reset-password', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ empid: empid, newPassword: newPwd, token: sessionStorage.getItem('hi_english_admin_token') || '' })
  }).then(function(resp) { return resp.json(); }).then(function(data) {
    if (data.success) {
      var users = HiEnglish.getUsers();
