...
FILES = {
    'server.py': open('server.py', 'rb').read(),
...
# 5. Create commit
commit_msg = '''fix(j+server): 根治跨终端数据丢失——服务端启动不再覆盖本地进度

根因：init_data_files() 每次启动用 GitHub 数据直接覆盖本地 study_data.json。
如果写穿 GitHub 失败/延迟，下次重启就会用旧空数据覆盖客户端刚推送的进度。

修复：
1. server.py: init_data_files 对 study_data.json 改为逐用户合并（非覆盖）
2. server.py: push_study_data_immediate 加重试(3次x12秒超时)
3. common.js: pushServerStudyDataImmediate + startPeriodicSync(60s)
4. student.js: 打卡完成立即推送 + 5秒兜底重拉对比更新
5. student.html: 移除报告页安装卡片
6. SW v40/core-v39'''
